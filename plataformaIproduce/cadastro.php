...
<?php
        ...
        if(isset($_GET['btnEnviar'])){
                ...
                $sqlConfereExistencia = "select usuario from tblusuarios where usuario = '$email'";

                $script = mysqli_query($conexao,$sqlConfereExistencia);
                // echo($sqlConfereExistencia);
                if(mysqli_num_rows($script) > 0){
                        // echo("entrou if");
                               
                                $placeholder= "Email existente, favor digitar outro";
                }else{
